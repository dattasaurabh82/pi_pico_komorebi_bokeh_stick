...
/*
  1. Adafruit Board: DVIGFX16 display(DVI_RES_400x240p60, adafruit_feather_dvi_cfg);
  2. Pimoroni Pico DV: DVIGFX16 display(DVI_RES_320x240p60, pimoroni_demo_hdmi_cfg);
  3. Pico DVI Sock: DVIGFX16 display(DVI_RES_320x240p60, pico_sock_cfg);

  - First argument: DVI_RES_400x240p60: 320x240 16-bit color framebuffer (Double-buffering is not an option in 16-bit color mode, just not enough RAM; all drawing
  operations are shown as they occur.)
  - Second argument: hardware configuration (easily switchable)

  Notes: With the Feather config, TMDS goes to the wrong GPIOs(D0± = GP12/13, CK± = GP14/15, D2± = GP16/17, D1± = GP18/19.) → black screen.
  
  Notes: 
    1. begin() overclocks sysclk to 252 MHz itself; if it fails, the onboard LED blinks at ~1 Hz instead of showing video.
    2. A 400x240 mode is possible but pushes overclocking even higher than 320x240 mode. 
    SOME BOARDS MIGHT SIMPLY NOT BE COMPATIBLE WITH THIS.
    May require selecting QSPI div4 clock (Tools menu) to slow down flash accesses, 
    May require further over-volting the CPU to 1.25 or 1.3 V.
*/
