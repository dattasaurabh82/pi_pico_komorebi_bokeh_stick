...
static void deal_dapples() {
  int n_large = (N_DAPPLES * 3 + 8) / 16; if (n_large < 1) n_large = 1;
  int n_mid_end = n_large + (N_DAPPLES * 5 + 8) / 16;
  for (int i = 0; i < N_DAPPLES; i++) {
    float base;
    if (i < n_large)        base = frand(110, 150); // large
    else if (i < n_mid_end) base = frand(65, 95);   // mid
    ...
    if (i < n_mid_end) { d[i].bri = frand(0.40f, 0.80f); d[i].bf = frand(0.010f, 0.030f); }
